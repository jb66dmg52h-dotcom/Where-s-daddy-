...
lv_obj_t *createHomeScreen() {
  ...
  const lv_color_t accent = lv_color_hex(0x58E83B);
  lv_obj_t *panel =
      Ui::createHeroPanel(screen, "AT HOME", "GUELPH, ON", accent, lv_color_hex(0x105B16), lv_color_hex(0x208328));
  lv_obj_t *icon = Icons::createHomeIcon(panel, accent);
  Ui::createIconBadge(panel, icon, accent);

  Ui::createInfoStrip(screen, "NOT MOVING", "0 km/h", "HOME", "11:23 AM", accent);
